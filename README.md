...
- **Per-frame timing**: the playback bar has a "Frame hold (ms)" field next to the global Delay — it overrides just the currently-viewed frame's hold time (e.g. a longer pause on a title card), leaving every other frame on the global Delay. "Use Default" clears the override. These per-frame overrides round-trip through JSON save/load and Presets, are covered by Undo/Redo, and Web Serial streaming honors them too. **They are not currently reflected in exported Arduino sketches** (`ANIM.ino`/`.h`) — those still use one fixed `delay()` for every frame; a known gap, not an oversight.
...
- Web Serial Streaming: Connect to your board via the Web Serial API (Chrome/Edge), then **Start Streaming**. **Disconnect** (or **Stop Streaming**) releases the port/stops the loop. If the device is unplugged mid-session, the app detects it and resets the connection state automatically.
  - **Important hardware note, learned the hard way**: on this cube, the Arduino does not drive the LEDs directly — the cube has its own onboard controller that does the actual multiplexing in its own firmware, and it falls back to its own built-in demo pattern whenever it stops receiving a live, continuous feed. The Arduino's job is just to relay bytes from your PC straight to that controller in real time — flash it with the **Live Relay Sketch** (Export tab), which does nothing but forward every byte it receives, unmodified: no parsing, no checksum, no ACK/NACK. (An earlier version of this app assumed the Arduino drove the LEDs directly and built a whole request/confirm protocol for that — checksums, ACKs, retries, a `displayFrame()` placeholder to fill in. All of that was solving the wrong problem for this hardware and has been removed.)
  - Because the controller reverts to its demo pattern the moment the feed stops, **streaming is continuous, not a one-shot send** — matching exactly how the exported `ANIM.ino` behaves (open handshake once, then `0xF2` + 64 raw bytes per frame forever, looped, and it never sends the close command). Start Streaming keeps looping your current animation (respecting per-frame timing) until you click Stop.
  - The protocol is: open command (70 bytes of `0xAD`, sent once), then repeating frames (`0xF2` + 64 raw bytes, no checksum), for as long as streaming is active. The close command (`0xED`) is intentionally never sent while streaming, since that appears to be what tells the controller to give up and show its own demo.
  - If you edit the animation while streaming, those changes won't appear until you stop and start streaming again — the loop snapshots the animation when it starts.
  - Opening a serial port from a browser toggles the DTR line, which auto-resets most Arduino Uno/Nano-style boards. Connect waits ~2 seconds after opening the port before it's usable, to ride out that reset.
